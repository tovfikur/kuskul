...
* `InventoryItemDrawer`
  ...
  * Sections: Item fields (sku/name/uom/reorder/is\_active), audit (created\_at).
...
* `LocationsManagement` (sub-panel in Inventory)
  ...
  * CRUD for `InventoryLocation` (code/name/is\_active).
...
* `PurchaseOrderDrawer`
  ...
  * Lines grid: item, qty\_ordered, unit\_price.

  * Actions: Save (open), Create Receipt, Cancel.

* `GoodsReceiptDrawer`

  * Fields: PO, received\_at, received\_by, location.

  * Lines: PO line → qty\_received.
...
* `AssetDrawer`
  ...
  * Fields: tag, serial\_no, category, location, custodian, status.
